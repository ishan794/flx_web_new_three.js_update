...
for line in open(transcript_path, 'r', encoding='utf-8'):
    try:
        ...
        # We stop before step 600 which is roughly today's session where we added parallax
        if step.get('type') == 'PLANNER_RESPONSE' and step.get('step_index') < 550:
...
for k, v in files.items():
    if "home.blade.php" in k or "app.blade.php" in k or "style.css" in k:
        ...
        with open(k + ".restored", "w", encoding='utf-8') as f:
            f.write(v)
